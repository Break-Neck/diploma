...
template <typename T>
constexpr const sherwood_v3_entry_constexpr<T>
    EntryDefaultTable<T>::table[min_lookups];
...
class sherwood_v3_table : private EntryAlloc, private Hasher, private Equal {
 ...
 public:
  ...
  void rehash(size_t num_buckets) {
    ...
    for (EntryPointer it = new_buckets,
                      real_end = it + static_cast<ptrdiff_t>(
                                          num_buckets + new_max_lookups - 1);
         it != real_end; ++it) {
  ...
  size_t bucket_count() const { return num_slots_minus_one + 1; }
 ...
 private:
  ...
  EntryPointer entries =
      const_cast<Entry *>(reinterpret_cast<const Entry *>(DefaultTable::table));
  ...
  void deallocate_data(EntryPointer begin, size_t num_slots_minus_one,
                       int8_t max_lookups) {
    if (begin != const_cast<Entry *>(
                     reinterpret_cast<const Entry *>(DefaultTable::table))) {
...
struct prime_number_hash_policy {
  ...
  void commit(uint8_t new_prime_index) { prime_index = new_prime_index; }
  void reset() { prime_index = 0; }
